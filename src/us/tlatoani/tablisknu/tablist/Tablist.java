...
public class Tablist {
    ...
    public void enableScores() {
        if (!areScoresEnabled()) {
            ...
            createPacket.setDisplayName(WrappedChatComponent.fromText(OBJECTIVE_NAME));
    ...
    public void disableScores() {
        if (areScoresEnabled()) {
            ...
            removePacket.setDisplayName(WrappedChatComponent.fromText(OBJECTIVE_NAME));
    ...
    public void sendPacket(PacketContainer packet, Object exceptLoc) {
        target.filter(Player::isOnline).ifPresent(player -> PacketUtil.sendPacket(packet, exceptLoc, player));
    }
